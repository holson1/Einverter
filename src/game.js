...
var playState = {
    ...
    create: function() {
        ...
        player.ready = false;
        // TODO: shoot should be outside of playState...
        player.shoot = playState.shoot;
        ...
        drawAnim.onComplete.add(playState.bowDrawn, this);
        ...
        orbs.spawnClock = 30;
        orbs.spawnOrb = playState.spawnOrb;
    ...
    update: function() {
        ...
        */
        ...
        game.physics.arcade.overlap(shots, orbs, playState.collisionHandler, null, this);
    ...
    collisionHandler: function(shot, orb) {
    ...
    bowDrawn: function(player, animation) {
        player.ready = true;
    },

    // handle shooting
    // player.shoot
    shoot: function(shots) {
        ...
        if (this.ready) {
            ...
            fire.play();
            playState.createShot(y, shots);
            this.ready = false;
        }
    },

    // create a new shot
    createShot: function(y, shots) {
        var shot = shots.create(0, y, 'shot', 0);
        shot.active = true;
        shot.critical = false;
        shot.body.velocity.x = 7000;
        shot.checkWorldBounds = true;
        shot.events.onOutOfBounds.add(playState.destroySprite, this);
    },
    ...
    spawnOrb: function() {
        ...
        anim.onComplete.add(playState.destroySprite, this);
        ...
        orb.events.onOutOfBounds.add(playState.destroySprite, this);
...
// TODO: move this and playState to a separate file
var gameOverState = {
    create: function() {
